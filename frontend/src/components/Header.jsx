...
import { Plane, BarChart3, FileText, ChevronRight, Activity, Layers, Sparkles } from 'lucide-react';

export default function Header({ activeView, setActiveView, onOpenWhitepaper }) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-slate-950/90 backdrop-blur-md border-b border-slate-800/80 py-3 shadow-2xl shadow-sky-950/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        
        {/* Brand / Logo */}
        <div 
          className="flex items-center space-x-3 cursor-pointer" 
          onClick={() => { setActiveView('overview'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
        >
