...
def fetch_cleaned_observations(supabase: Client, table_name: str = "cleaned_observations_table") -> pd.DataFrame:
    """
...
def push_to_supabase(
    ...
    """
    Idempotently inserts the computed index payload into the Supabase 'index_values' table.
    ...
    Idempotency guarantee:
      ...
      3. Insert the newly computed records in batches of 500.
    ...
    # ------------------------------------------------------------------
    # Step 1: Idempotency — delete existing rows for affected dates ONCE
    # ------------------------------------------------------------------
    ...
    for obs_date in observation_dates:
        supabase.table(table_name).delete().eq("observation_date", obs_date).execute()
    logger.info("Existing rows deleted. Proceeding with fresh insert.")

    # ------------------------------------------------------------------
    # Step 2: Insert in batches of 500
    # ------------------------------------------------------------------
    # Canonical key set — aligned exactly to create_index_table.sql
    SCHEMA_KEYS = [
        "observation_date",
        "base_period_date",
        "index_type",
        ...
        "data_provenance_mix",
        # calculated_at intentionally absent — DB DEFAULT handles it
    ]
    ...
    for i in range(0, len(payload), batch_size):
        ...
        # Strip payload to only the known schema keys
        clean_batch = [
            {k: item.get(k) for k in SCHEMA_KEYS}
            for item in batch
        ]
        ...
        try:
            ...
            logger.error(f"Batch insert failed for batch {i // batch_size + 1}: {insert_err}")
            raise
    ...
    try:
        ...
        if df_observations.empty:
            logger.error(
                ...
                "Ensure the cleaning pipeline has been run first (main.py --run-cleaning)."
            )
            return
        ...
        # Step 5: Idempotently push to Supabase 'index_values' table
